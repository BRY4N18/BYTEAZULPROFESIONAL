...
namespace BYTEAZULPROFESIONAL
{
    public partial class fmCaja : Form
    {
        ...
        private void llenartxttex()
        {
            ...
            suscriptor = new CsSuscripciones(); double total = 0;
            if (suscriptor.VerificarSuscriptor(txtIdCliente.Text))
            {
                if (suscriptor.VerificarSuscriptorAldia(txtIdCliente.Text))
                {
                    total += (subtotal + iva);
                    total *= 0.75;
        ...
        private void btnGenerarFactura_Click(object sender, EventArgs e)
        {
            if (com())
            {
                ...
                if (caja.Generarfactura(dgvDetallesVentas))
                {
                    ...
                    txtTotal.BackColor = SystemColors.Window; txtSubtotal.Clear();txtIva.Clear(); txtTotal.Clear(); txtCambio.Clear(); txtPago.Clear();
                }
