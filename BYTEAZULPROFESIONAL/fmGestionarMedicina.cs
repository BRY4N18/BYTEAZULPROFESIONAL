...
namespace BYTEAZULPROFESIONAL
{
    public partial class fmGestionarMedicina : Form
    {
        ...
        private void fmGestionarMedicina_Load(object sender, EventArgs e)
        {
            ...
            DataTable VerMedicina = conexion.Insertinto("Select M.id_producto Id, M.md_producto Medicina, M.md_categoria Categoria, M.id_proveedor [Id Proveedor], P.prv_nombre [Nombre del proveedor], M.id_iva Iva, M.md_descripcion Descripcion, M.md_precio_unidad [Precio unitario], M.md_estado Estado, sum(l.lt_cantidad) Cantidad from Medicina M left join Lotes L on M.id_producto=L.id_producto inner join Proveedores P on M.id_proveedor=P.id_proveedor where L.lt_fecha_caducidad>GETDATE() group by M.id_producto, M.md_producto, M.md_categoria, M.id_proveedor, M.id_iva, M.md_descripcion, M.md_precio_unidad, M.md_estado, P.prv_nombre");
            ...
            dgvVerMedicina.Columns["Id"].Visible = false;
            dgvVerMedicina.Columns["Id Proveedor"].Visible = false;
        }
        ...
        private void dgvVerMedicina_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == dgvVerMedicina.Columns["Editar"].Index && e.RowIndex >= 0)
            {
                ...
                IdMedicina = dgvVerMedicina.Rows[fila].Cells["Id"].Value.ToString();
                NombreMedicina = dgvVerMedicina.Rows[fila].Cells["Medicina"].Value.ToString();
                Categor = dgvVerMedicina.Rows[fila].Cells["Categoria"].Value.ToString();
                IdProveedor = int.Parse(dgvVerMedicina.Rows[fila].Cells["Id Proveedor"].Value.ToString());
                Iva = dgvVerMedicina.Rows[fila].Cells["Iva"].Value.ToString();
                Descripcion = dgvVerMedicina.Rows[fila].Cells["Descripcion"].Value.ToString();
                Precio = double.Parse(dgvVerMedicina.Rows[fila].Cells["Precio unitario"].Value.ToString());
                Estado = dgvVerMedicina.Rows[fila].Cells["Estado"].Value.ToString();
